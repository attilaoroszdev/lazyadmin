...
*This file had been formatted to be viewed with lynx/pandoc on a Linux CLI. Your mileage may vary with other software, especially when it comes to code-blocks.*
...
- `tabXitemXsubX` - for sub menu entries

where `X` stands for number of either tab, menu item, or submenu-item, respectively.
...
* `funct01` means the second menu-entry on the first tab (`tab 0`, `item 1`)
...
For sub-menus, there is an additional number for sub-menu entry, making the `subfunctXXX` takes three numbers to be identifiable. The first sub-menu entry on the first menu entry of the first tab will be `subfuncts000` (`tab 0`, `menu 0`, `submenu 0`), while the second sub-menu entry on the third menu of the second tab would be `subfuncts121` (tab1, menu2, submenu1).
...
There is one catch. At the moment, sub-menus would only (correctly) work, when invoked from the main menu... To do that, you would call:
